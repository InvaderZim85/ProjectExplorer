...
# Main entry point
# ===========================
$projectFile = "ProjectExplorer\ProjectExplorer.csproj"
...
try {
    ...
    $output = Join-Path -Path $currentLocation -ChildPath "ProjectExplorer\bin\"
    ...
    # Create the release
    & $dotnet publish "ProjectExplorer.sln" -p:PublishProfile=ProjectExplorer/Properties/PublishProfiles/FolderProfile

    # Create the release archive
    $sourceDir = Join-Path -Path $currentLocation -ChildPath "ProjectExplorer\bin\Release\net8.0-windows\publish\win-x64\*"
    ...
    $targetPath = Join-Path -Path $releaseDir -ChildPath "ProjectExplorer_v$newVersion.zip"
